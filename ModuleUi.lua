...
-- ========== CONFIG SYSTEM ==========
Library.ConfigFolder = "X2Moji - Hub/Config"
Library.ScriptName = "Unknown"
Library.ConfigData = {}
Library.Elements = {}
...
local function EnsureConfigFolder()
	...
	if not isfolder(Library.ConfigFolder) then
		if not isfolder("X2Moji - Hub") then
			makefolder("X2Moji - Hub")
		end
...
local function GetConfigPath()
	...
	local fileName = player.DisplayName .. "_" .. player.UserId .. ".json"
	return Library.ConfigFolder .. "/" .. fileName
end
...
function Library:SaveConfig()
	...
	local data = {
		ScriptName = Library.ScriptName,
		Settings = Library.ConfigData
	}
...
function Library:LoadConfig()
	...
	if success and data and data.ScriptName == Library.ScriptName then
		Library.ConfigData = data.Settings or {}
		-- Apply loaded values to elements
...
function Library:Window(p)
	...
	function Tabs:Tab(p)
		...
		function Func:Slider(p)
			...
			local function updateSlider(value, skipSave)
		...
		function Func:Dropdown(p)
			...
			-- Wrap callback to add auto-save
			local function wrappedCallback(v)
				currentValue = v
				pcall(Callback, v)
				if Flag then
					Library.ConfigData[Flag] = v
					Library:SaveConfig()
				end
			end
			...
			local DropdownSelect = addDropdownSelect(Dropdown, Dropdown, Multi, wrappedCallback, Value, List)
		...
		function Func:Keybind(p)
			...
			local function change(skipSave)
				...
				-- Auto save config
				if Flag and not skipSave then
					Library.ConfigData[Flag] = {Key = tostring(Key), Value = Value}
					Library:SaveConfig()
				end
			end
			...
			-- For config system
			function New:Set(data)
				if typeof(data) == "table" then
					if data.Key then
						Key = Enum.KeyCode[data.Key:gsub("Enum.KeyCode.", "")] or Enum.KeyCode.E
						TextLabel_1.Text = tostring(Key):gsub("Enum.KeyCode.", "")
						adjustBoxBindSize()
					end
					if data.Value ~= nil and Value ~= data.Value then
		...
		function Func:Textbox(p)
			...
			local function o(skipSave)
				if #TextLabel_1.Text > 0 then
					pcall(Callback, TextLabel_1.Text)
					-- Auto save config
					if Flag and not skipSave then
						Library.ConfigData[Flag] = TextLabel_1.Text
						Library:SaveConfig()
					end
				end
			end
